...
function onCreate()
	...
	precacheImage('characters/' .. name_of_character_xml);
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if curStep >= 816 and curStep <= 847 then
		doIdle = false
...
function onBeatHit()
	...
	if curStep >= 798 and curStep <= 807  and noAnimated then
		objectPlayAnimation(name_of_character, 'idle');
		setProperty(name_of_character .. '.offset.x', idleoffsets[1]);
		setProperty(name_of_character .. '.offset.y', idleoffsets[2]);
	end

	if curStep >= 808 and curStep < 848 then
...
function onTweenCompleted(tag)
	if tag == 'tweenYTails' then
		noAnimated = false
		playAnim(name_of_character,'transition2',true)
	end
end
...
function onUpdate(elapsed)
	if curStep == 798 then
		...
		doTweenY('tweenYTails',name_of_character,getProperty(name_of_character..'.y')-500,0.7,'quadOut')
	end
