...
function GodTrail:makeTrailFrame()
	...
	if self.target == 'gf' or self.target == 'bf' or self.target == 'boyfriend' or self.target == 'dad' then
		order = getObjectOrder(self.target..'Group') - 1;
	else
		order  = getObjectOrder(self.target) - 1
	end
	...
	local href = self.href;
		...
		if self.target == 'gf' or self.target == 'bf' or self.target == 'boyfriend' or self.target == 'dad' then
			...
			makeAnimatedLuaSprite(sprName, 'characters/zynux_assets', 800, 220)
		end
...
function onEvent(name, value1, value2)
	if (name == 'Toggle Godly Trail') then
		...
		for i = 1,#chars,1 do
			if (isAny(chars[i], 'bf', 'gf', 'dad', 'zynux') and value2:len() > 0) then
				sparkTrail(chars[i]);
	...
	elseif (name == 'Set Trail Color') then
		local chars = value1:split(',', true);
		for i = 1,#chars,1 do
			if (isAny(chars[i], 'bf', 'gf', 'dad', 'zynux')) then --table.unpack(targets) --broken?
				sparkTrail(chars[i]);
