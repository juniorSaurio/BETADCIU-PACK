...
void main()
{
    ...
    float gl1 = glitch(uv, LINES1, floor(uTime*2.0));
    float gl2 = glitch(uv, LINES2, floor(uTime*5.0));
    float gl3 = glitch(uv, LINES3, floor(uTime));
    ...
    if (intensity > 0.5) {
        gl_FragColor = mix(gl_FragColor, vec4(hash(uvec3(fragCoord.xy, uTime * 100.0)), 1), (intensity - 0.5) / 0.5);
